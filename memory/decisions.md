...
| 2026-03-29 | Discord 数据分级存储架构 | 单频道历史消息可达76万条，纯 git 存储不可持续。方案：原文保留30天在 git；超出30天时打包上传 GitHub Releases + 同步调用 Claude API 生成当月月报存入 monthly_reports/YYYY-MM.md；每日纯统计摘要永久留 git。归档与月报生成合并为同一 workflow 步骤，周期一致 | news/discord |
