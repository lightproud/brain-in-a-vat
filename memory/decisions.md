...
| 2026-03-29 | Agent 改为直接推 main，取消分支+PR 流程 | main 有保护规则，git 可 revert，手动合并是不必要瓶颈 | 全局 |
| 2026-03-29 | main 分支添加 Ruleset 保护规则（禁止删除） | 防止误操作删除主干 | 全局 |
| 2026-03-29 | 前台专岗不固定编制，按需增设 | 专岗是上下文管理策略，不是固定职位。"美术总监"不再作为固定岗位，按需临时开设 | 全局 |
...
2. 各子项目在自己的分支上独立开发
3. ~~功能稳定后，由主控制台创建 PR 合并回 main~~ **已更新（2026-03-29）：Agent 直接推 main，取消 PR 流程**
