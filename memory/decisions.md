...
| 2026-03-29 | 建立 memory/lessons-learned.md 经验教训文档 | 将踩坑经验固化为可检索记录，避免重复犯错 | 全局 |
| 2026-03-29 | 各子项目 CONTEXT.md 添加启动验证清单 | 新会话启动时逐项确认环境，减少因遗漏检查导致的错误 | news/wiki/game |
| 2026-03-29 | CLAUDE.md 引入 Plan/Execute 两阶段约定 | 中等以上复杂度任务必须先规划后执行，避免方向性返工 | 全局 |
| 2026-03-29 | 聚合器空结果时保留旧 news.json | 防止全源失败时覆盖有效数据，前端展示空白 | news |
| 2026-03-29 | 建立 .claude/commands/ 自定义命令目录 | 将高频操作封装为可复用的 slash command，降低操作门槛 | 全局 |
| 2026-03-29 | methodology.md 新增专岗分类策略章节 | 明确 Chat/Code 两类角色的职责边界，指导多会话协作 | 全局 |
| 2026-03-29 | Issue 积压定期清理机制 | 通过主控台 Issue（如 #47）批量审查 open Issues，防止积压失控 | 全局 |
| 2026-03-29 | 决策日志覆盖所有关键配置变更 | 不仅记录架构决策，也记录流程改进、安全策略等操作性决策 | 全局 |
...
| `claude/wiki-deploy-github-pages-6buag` | wiki | 数据修复 + 50 页内容填充 + GitHub Pages 部署（PR #20 待合并） |
