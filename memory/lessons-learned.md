...
- **Context**：本轮会话做了多个重要决策（废弃分支工作流、新增 Code-site、确立全 main 推送），以及踩了多个坑（deploy 冲突、Issue 传递失效、规则未传播到新会话）
- **Problem**：主控台只在被提醒时才更新 decisions.md 和 lessons-learned.md，不会主动沉淀。每次都需要制作人反复提醒"总结经验"
- **Fix**：主控台在每个决策做出后、每次踩坑修复后，必须**立即**更新对应的 memory 文件。不等提醒，不攒着批量写。这是主控台的核心职责之一
- **Impact**：知识传承效率、制作人管理负担
